...
void loop() {
    ...
    // 4. Distance Calculation (PORTED FROM OPENCV) & Command Processor
    long send_area = 0;
    int distance_cm = 0; 
    ...
    if (count > min_blob_area) {
        ...
        // Using your exact 512,000 math here:
        if (send_area > 0) {
        ...
        if (state == LOCKED || state == LOST_BUT_RETAINING) {
            ...
            // Retain distance memory using your math:
            if (send_area > 0) {
                distance_cm = (int)sqrt(512000.0 / send_area);
            }
